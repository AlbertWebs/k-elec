...
<div class="space-y-6">
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <form action="{{ route('admin.banners.update', $banner) }}" method="POST" class="space-y-6" enctype="multipart/form-data">
    @csrf
    @method('PUT')
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div class="space-y-4">
            <div>
                <label for="name" class="block text-sm font-medium text-gray-700 mb-1">Banner Name *</label>
                <input type="text" id="name" name="title" required
                       class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                       value="{{ old('title', $banner->title) }}" placeholder="Enter banner name">
                @error('title')
        ...
        <div class="space-y-4">
            <div>
                <label for="position" class="block text-sm font-medium text-gray-700 mb-1">Banner Position *</label>
                <input type="text" id="position" name="position" required
                       class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                       value="{{ old('position', $banner->position) }}" placeholder="Enter banner position">
                @error('position')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <input type="url" id="url" name="url" required
                       class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                       value="{{ old('url', $banner->url) }}" placeholder="Enter banner URL">
                @error('url')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div class="space-y-6">
            <div class="space-y-4">
                <h4 class="text-md font-medium text-gray-700">Current Banner</h4>
                ...
                @if($banner->image)
                    <div class="space-y-2">
                        ...
                        <div id="existing-main-image" class="relative group">
                            <img src="{{ url('/') }}/storage/{{ $banner->image }}" alt="{{ $banner->title }}" 
                                 class="w-32 h-32 object-cover rounded-lg border" data-image-path="{{ $banner->image }}">
