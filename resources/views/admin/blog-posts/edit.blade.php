        ...
        <form action="{{ route('admin.blog-posts.update', $blogPost) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div>
                ...
                <textarea id="excerpt" name="excerpt" rows="2"
                          class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-red-500">{{ old('excerpt', $blogPost->excerpt) }}</textarea>
            </div>
            ...
            <div> font-mono text-sm
                ...
                <textarea id="content" name="content" rows="10" required
                          class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-red-500">{{ old('content', $blogPost->content) }}</textarea>
                @error('content')
                    <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <div class="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:border-gray-400 transition-colors cursor-pointer" onclick="document.getElementById('featured_image').click()">
                    <div id="image-preview" class="{{ $blogPost->featured_image ? '' : 'hidden' }} mb-4">
                        <img id="preview-img" src="{{ $blogPost->featured_image ? Storage::url($blogPost->featured_image) : '' }}" alt="Preview" class="max-w-xs h-48 object-cover rounded-lg mx-auto">
                    </div>
                    <div id="upload-placeholder" class="{{ $blogPost->featured_image ? 'hidden' : '' }}">
            ...
            <div class="flex items-center space-x-6">
                <label class="flex items-center">
                    <input type="checkbox" id="is_published" name="is_published" value="1" class="rounded border-gray-300 text-red-600" {{ old('is_published', $blogPost->is_published) ? 'checked' : '' }}>
